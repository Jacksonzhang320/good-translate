...
class RunStateTests(unittest.TestCase):
    ...
    def test_unrelated_global_term_does_not_retranslate_every_chunk(self):
        tmp, temp_dir = self._workspace()
        with tmp:
            glossary = glossary_doc()
            glossary["high_frequency_top_n"] = 2
            self._write(temp_dir / "glossary.json", json.dumps(glossary, ensure_ascii=False))
            run_state.record_chunks(str(temp_dir), ["chunk0001", "chunk0002"])
            glossary["terms"].append({
                "id": "Alpha", "source": "Alpha", "target": "阿尔法",
                "category": "concept", "aliases": [], "gender": "unknown",
                "confidence": "low", "frequency": 99, "evidence_refs": [], "notes": "",
            })
            self._write(temp_dir / "glossary.json", json.dumps(glossary, ensure_ascii=False))
            plan = run_state.plan(str(temp_dir))
