...
### 3. Reference Consistency, Boundary Isolation & GB/T 7714 Standards
...
- **Post-Reference Boundary Isolation Gate**: When a paper features content following references (STAR+Methods, Extended Methods, Appendices), verify that references strictly close at the final citation. Ensure subsequent body paragraphs are rendered in full-size 16pt body style, never remaining in 10pt hanging-indent reference style.

### 4. Zero Red Math Error Gate (公式标红零容忍硬门禁)
- **Automated Playwright Check**: Verify the build executed Playwright DOM verification asserting zero elements matching `mjx-container mjx-merror, mjx-container [data-mjx-error], mjx-container [mathcolor="red"], mjx-container [fill="red"], mjx-container [stroke="red"]`. Any red syntax error aborts publication.
- **Dual-Side Math Hygiene**: In bilingual editions, ensure LaTeX expressions in both English original and Chinese translation blocks are valid and free of corrupt OCR tokens (e.g. `\dag`, unclosed macros).

### 5. Terminology Consistency (零术语漂移终审)
...
### 6. End-matter & Appendices Delineation
...
### 7. Multi-Scale Visual Inspection via `view_file` (多尺度视觉质检)
The Agent MUST use `view_file` to visually review both macro contact sheets and high-resolution 1:1 pages:
- Title page (sheet 1);
- Pages with dense mathematical formulas (confirming 0 red error glyphs and crisp notation at full scale);
- First figure / table page (layout balance, caption positioning);
- Reference list entry page and final reference termination page (verifying clean boundary before Methods);
