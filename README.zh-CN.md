...
PDF 输入
  │
  ▼
预检 → 几何解析 / MinerU / OCR + MinerU
  │
  ▼
块级 doc.json + 测量得到的 style.json + 源 PDF 对象裁图
  │  v2 manifest 完整追踪源文件、块、chunk 与 hash
  ▼
解析视觉验收 → 可续跑并行 chunk 翻译
  │  每次派发固化语言、风格指令、术语表和源文本
  ▼
合并观察 → 精确术语纠正 → 冻结
  │
  ▼
版本化浏览器渲染 → DOM 检查 → 页面联系表 → 视觉验收
```
...
- **保持源页风格** — 页面、字体、标题、栏数风格来自源 PDF；独立公式与关键视觉对象保留源图裁片
- **三种出版版本** — 默认一键生成三种标准出版格式：
  1. **期刊原版**（`{Title}_期刊原版.pdf`）：双栏/单栏原汁原味排版；
  2. **双语对照版**（`{Title}_双语对照.pdf`）：段落中英对齐交错排版；
  3. **党政公文版**（`{Title}_公文版.pdf`）：严格遵循 **GB/T 9704—2012**（小标宋红头、红线、标准字号行距）与 **GB/T 7714**（参考文献 10pt 紧凑悬挂缩进）。
- **文献元数据主动检索门禁** — 坚决拒绝被动代码回退与随机哈希占位符（如“学术期刊译情参阅 编号：39e516e484da”）。Agent 具备主动检索能力，当文献元数据缺失或不确定时自动发起联网或学术检索，精准查证正式发表期刊（如《自然·神经科学》）、发文年份与正式 DOI，规范渲染公文红头发文字号。
- **公式标红零容忍硬门禁 (v2.2)** — Playwright 渲染层深度扫描 MathJax DOM 树（`mjx-merror`, `[mathcolor="red"]`, `[data-mjx-error]`），主动拦截任何因 OCR 误识导致的微观 LaTeX 语法损坏并实行双端自动净化，确保全篇公式 0 报错；
- **后置方法附录隔离门禁 (v2.2)** — 深度识别 Cell/Neuron 等顶刊“参考文献后置实验方法（STAR Methods / Appendices）”结构，自动闭合参考文献状态机，后置正文段落严格恢复 GB/T 9704 三号仿宋（16pt）标准正文排版，彻底杜绝小字号悬挂缩进污染；
- **多尺度视觉质检验收规范 (v2.2)** — 坚决杜绝低分辨率缩略图盲审，实行“宏观概览联系表 + 1:1/2.0x 微观高清核验”双层视觉验收机制，在公式密集页与附录过渡页执行切片级人工与模型审查；
- **规范语义化命名** — 输出自动提取核心书名并附加版式后缀，保留历史别名兼容。
- **双验收门** — 解析和发布都不能仅凭文件存在或自动渲染就宣告完成
- **多格式输出** — 支持 HTML/PDF 确定性排印；按需追加 DOCX/EPUB
- **可选输出控制** — 显式 EPUB 封面、自定义 temp root、面向用户的导出别名
- **多语言** — zh、en、ja、ko、fr、de、es（可扩展）
- **多格式输入** — PDF 使用可靠保真路径；DOCX/EPUB 使用兼容转换路径

## 前置要求

- **Agent 运行环境** — Codex、Claude Code 或 OpenClaw，已安装并可运行 Skill
- **uv + Python 3.12** — 依赖锁定在 `uv.lock`，使用 `uv sync`
- **Edge/Chromium** — 用于确定性的 HTML 检查与 PDF 打印
- **MinerU** — 公式多或双栏 PDF 必需
- **OCR 依赖** — 扫描 PDF 需要 OCRmyPDF、Tesseract 语言包和 Ghostscript
- **Calibre/Pandoc** — 仅 DOCX/EPUB 兼容输入或按需输出这些格式时需要
...
可靠 PDF 成品记录在 `<run>/build_result.json`，实际文件位于 `<run>/publish/<render-hash>/`：

| 文件 | 说明 |
|------|------|
| `book.html` | 保持源页风格的中文网页版 |
| `book.pdf` | 保持源页风格的中文 PDF |
| `book_bilingual.html` | 原文紧跟译文的双语网页版 |
| `book_bilingual.pdf` | 原文紧跟译文的双语 PDF |

`generated` 只表示机械检查通过；`accepted` 才表示页面联系表已被逐项审阅并记录具体证据。以 `scripts/pipeline.py status <run>` 为完成判定。
...
**兼容路径说明：** `{book_name}_temp/` 只对应一次 DOCX/EPUB 兼容运行。可靠 PDF 路径使用新的短 run id 和按内容寻址的发布目录，不通过删除旧证据来强制重建。
...
| `scripts/pipeline.py` | 可靠 PDF 预检、解析/派发/发布门禁、状态、验收与清理 |
| `scripts/pdf_document.py` | PDF/MinerU/OCR 几何 → 块级文档、测量样式与源裁图 |
| `scripts/publish.py` | 按内容寻址的源风格 HTML/PDF 发布器 |
| `scripts/qa_preview.py` | 源页、对象和成品页面联系表 |
