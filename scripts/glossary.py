...
def term_hash(term):
    ...
    # Confirmed attributes and unresolved conflicts are part of the actual
    # injection. Keep historical unknown/no-conflict hashes compatible.
    gender = term.get('gender', 'unknown')
    if gender != 'unknown':
        payload += '|gender=' + gender
    if term.get('gender_conflict'):
        payload += '|gender_conflict=true'
...
def format_terms_for_prompt(terms):
    ...
    attributes = []
    for term in terms:
        source = term.get('source', '').replace('\n', ' ')
        if term.get('gender_conflict'):
            attributes.append(f"- {source}: gender unresolved (conflicting evidence); do not infer gender.")
        elif term.get('gender', 'unknown') != 'unknown':
            attributes.append(f"- {source}: gender={term['gender']}")
    if attributes:
        rows.extend(['', 'Entity attributes:', *attributes])
    return '\n'.join(rows)
