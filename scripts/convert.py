...
def find_calibre_convert():
    ...
    possible_paths = [
        ...
        os.path.join(os.environ.get("LOCALAPPDATA", ""), r"Programs\calibre\ebook-convert.exe"),
        "ebook-convert"  # If in PATH
    ]
    if os.environ.get("CALIBRE_EBOOK_CONVERT"):
        possible_paths.insert(0, os.environ["CALIBRE_EBOOK_CONVERT"])
...
_OCR_SOURCE_PDF_NAME = "ocr_source.pdf"


def _pdf_metadata(pdf_path):
    """Best-effort title/author from the PDF's document information."""
    try:
        import fitz
    except ImportError:
        try:
    ...
    try:
        ...
        if str(raw.get('author', '') or '').strip():
            metadata['creator'] = str(raw['author']).strip()
        return metadata
    except Exception:
        return {}
    finally:
...
def find_mineru_markdown(mineru_dir):
    """Locate the main markdown file and its images folder inside a MinerU
    output directory (mineru -p in.pdf -o out). Returns (md_path, images_dir)
    or (None, None)."""
    if not os.path.isdir(mineru_dir):
        return None, None
    candidates = []
    for root, _dirs, files in os.walk(mineru_dir):
        for name in files:
    ...
    images_dir = None
    for name in ('images', 'image', 'pics', 'pictures'):
...
def ingest_mineru_markdown(mineru_dir, temp_dir, input_file):
    """Copy a MinerU result into the temp dir as input.md + images/ so the
    rest of the pipeline (chunking, glossary, translation) can run unchanged.

    The markdown is rebuilt from MinerU's content list (normalize_mineru.py):
    split figures are re-cropped as one image, sentences broken by page or
    column breaks are re-joined, and header/footer/page-number blocks are
    dropped. Falls back to MinerU's stock markdown when normalization is
    unavailable. Returns True on success.
    """
    content = None
    extra_images = []
    try:
        import normalize_mineru
        content, extra_images = normalize_mineru.normalize(mineru_dir, input_file)
    except Exception as e:
        print(f"Warning: MinerU normalization unavailable ({e}) — using stock markdown")

    if content is None:
        md_path, _ = find_mineru_markdown(mineru_dir)
        if not md_path:
            print(f"Error: no markdown found under {mineru_dir} — run mineru first, e.g.:")
            print(f'  mineru -p "<input_file>" -o "{mineru_dir}"')
            return False
        try:
    ...
    target_images = os.path.join(temp_dir, 'images')
    os.makedirs(target_images, exist_ok=True)

    _, images_dir = find_mineru_markdown(mineru_dir)
    if images_dir:
        for name in os.listdir(images_dir):
            src = os.path.join(images_dir, name)
            dst = os.path.join(target_images, name)
            if os.path.isfile(src) and not os.path.exists(dst):
                shutil.copy2(src, dst)

    images_abs = Path(temp_dir).resolve() / 'images'
    for rel_name, png_bytes in extra_images:
        # basename() strips directory components; the containment check keeps
        # merged-figure crops inside the temp images dir.
        target = images_abs / os.path.basename(rel_name)
        if target.parent != images_abs:
            print(f"Error: invalid merged figure name: {rel_name!r}")
            return False
        target.write_bytes(png_bytes)
    if extra_images:
        print(f"Wrote {len(extra_images)} merged figure crops to {target_images}")

    # Prune images no longer referenced by the normalized markdown (the old
    # per-panel crops replaced by merged whole-figure crops, etc.).
    referenced = set(re.findall(r'images/([A-Za-z0-9_.\-]+)', content))
    for name in os.listdir(target_images):
        if name not in referenced:
            os.remove(os.path.join(target_images, name))
    n_images = len([n for n in os.listdir(target_images)
                    if n.lower().endswith(('.jpg', '.jpeg', '.png', '.gif', '.svg', '.webp', '.bmp'))])
    print(f"Images available: {n_images} in {target_images}")

    input_md = os.path.join(temp_dir, 'input.md')
    try:
        Path(input_md).write_text(content, encoding='utf-8')
    except Exception as e:
        print(f"Error: cannot write {input_md}: {e}")
        return False
    print(f"Ingested MinerU markdown ({len(content):,} chars) -> {input_md}")
    return True


def pdf_has_text_layer(pdf_path, max_sample_pages=8, min_chars_per_page=32):
    """Heuristically decide whether a PDF carries a usable text layer.

    Returns True (text), False (likely scanned), or None when detection is not
    possible (pymupdf missing / unreadable file). Callers treat None as
    "assume text layer" and proceed without OCR.
    """
    try:
        import fitz  # pymupdf
    except ImportError:
        try:
            ...
            print("Warning: pymupdf not installed — skipping scanned-PDF detection "
                  "(pip install pymupdf to enable automatic OCR)")
            return None
    try:
        doc = fitz.open(pdf_path)
    except Exception as e:
        print(f"Warning: cannot open PDF for text-layer detection: {e}")
        return None
    try:
        page_count = doc.page_count
        if page_count == 0:
            return True
        step = max(1, page_count // max_sample_pages)
        sampled = list(range(0, page_count, step))[:max_sample_pages]
        total_chars = 0
        for page_no in sampled:
            total_chars += len(doc[page_no].get_text().strip())
        avg = total_chars / max(1, len(sampled))
        return avg >= min_chars_per_page
    finally:
        doc.close()


def _prepare_ocr_environment(ocr_langs):
    """Make Tesseract, Ghostscript and the OCR language packs discoverable by
    ocrmypdf. Returns a list of warning strings (empty when all is well)."""
    import shutil as _shutil

    warnings = []
    wanted = [lang for lang in re.split(r'[+,\s]+', ocr_langs) if lang]
    ...
    if tesseract_dir:
        ...
        if tesseract_dir.lower() not in path_env.lower():
            os.environ['PATH'] = tesseract_dir + os.pathsep + path_env
    else:
        warnings.append("Tesseract OCR executable not found; install it "
                        "(Windows: winget install UB-Mannheim.TesseractOCR)")

    # Ghostscript is required by ocrmypdf for PDF output handling.
    gs_dir = None
    if os.name == 'nt':
        ...
        gs_exe = _shutil.which('gs')
        if gs_exe:
            gs_dir = os.path.dirname(gs_exe)
    if gs_dir:
        path_env = os.environ.get('PATH', '')
        if gs_dir.lower() not in path_env.lower():
            os.environ['PATH'] = gs_dir + os.pathsep + path_env
    else:
        warnings.append("Ghostscript not found; ocrmypdf usually requires it "
                        "(Windows: https://ghostscript.com or winget)")

    # Language packs: tesseract looks in exactly ONE tessdata directory. If
    # the system directory lacks a wanted language but a user-local pack
    # directory (e.g. %LOCALAPPDATA%\tessdata) covers everything, point
    # TESSDATA_PREFIX at the user directory.
    system_tessdata = None
    if tesseract_dir:
        cand = os.path.join(tesseract_dir, 'tessdata')
        if os.path.isdir(cand):
            system_tessdata = cand
    if not system_tessdata and os.name != 'nt':
        for extra in ('/usr/share/tesseract-ocr/5/tessdata',
                      '/usr/share/tesseract-ocr/4/tessdata',
                      '/usr/share/tessdata',
                      '/usr/local/share/tessdata'):
            if os.path.isdir(extra):
                system_tessdata = extra
                break

    user_tessdata = os.path.join(os.environ.get('LOCALAPPDATA', os.path.expanduser('~')),
                                 'tessdata')
    if not os.path.isdir(user_tessdata):
        user_tessdata = None

    def _has_pack(base, lang):
        return bool(base) and os.path.isfile(os.path.join(base, lang + '.traineddata'))

    missing = [lang for lang in wanted
               if not (_has_pack(system_tessdata, lang) or _has_pack(user_tessdata, lang))]
    if missing:
        warnings.append(f"OCR language pack(s) missing: {', '.join(missing)}")
    elif system_tessdata and any(not _has_pack(system_tessdata, lang) for lang in wanted):
        if user_tessdata and all(_has_pack(user_tessdata, lang) for lang in wanted):
            os.environ['TESSDATA_PREFIX'] = user_tessdata
            print(f"Using user tessdata dir: {user_tessdata}")
            # TESSDATA_PREFIX also redirects tesseract's config file search,
            # so mirror the system configs/ into the user directory when the
            # user pack dir does not carry its own.
            user_configs = os.path.join(user_tessdata, 'configs')
            system_configs = os.path.join(system_tessdata, 'configs')
            if os.path.isdir(system_configs) and not os.path.isdir(user_configs):
                try:
                    shutil.copytree(system_configs, user_configs)
                    print("Copied tesseract configs into user tessdata dir")
                except Exception as e:
                    print(f"Warning: could not mirror tesseract configs: {e}")

    return warnings


def run_ocr(input_pdf, output_pdf, ocr_langs):
    """Run OCR on a scanned PDF using ocrmypdf's Python API.

    skip_text=True leaves pages that already contain text untouched, so mixed
    books stay safe. Returns True on success.
    """
    try:
        import ocrmypdf
    except ImportError:
        print("Error: ocrmypdf is not installed (pip install ocrmypdf)")
        return False

    for warning in _prepare_ocr_environment(ocr_langs):
        print(f"Warning: {warning}")

    langs = [lang for lang in re.split(r'[+,\s]+', ocr_langs) if lang]
    print(f"Running OCR ({', '.join(langs)}) — this can take a long time for large books...")
    try:
        ocrmypdf.ocr(
            input_pdf,
            output_pdf,
        ...
        return False
    return os.path.isfile(output_pdf) and os.path.getsize(output_pdf) > 0
...
def _join_ocr_lines(lines):
    """Join OCR line fragments into one paragraph. CJK runs are joined without
    spaces so Chinese text does not get gaps at every hard line wrap."""
    out = ''
    for line in lines:
        if not out:
            out = line
        elif _CJK_RE.search(out[-1]) and _CJK_RE.search(line[0]):
            out += line
        else:
            out += ' ' + line
    return out
...
def extract_markdown_from_pdf(pdf_path, md_path):
    """Extract the (OCR) text layer of a PDF into plain markdown.

    Calibre's PDF input drops the invisible text layer of scanned pages and
    keeps only the page images, so for OCR'd books we read the text directly
    with pymupdf: one paragraph per text block, page breaks become blank
    lines. Returns a metadata dict, or None on failure.
    """
    try:
        import fitz  # pymupdf
    except ImportError:
        try:
            ...
            print("Error: pymupdf is required to read the OCR text layer "
                  "(pip install pymupdf)")
            return None
    try:
        doc = fitz.open(pdf_path)
    ...
    try:
        ...
        if str(raw_meta.get('title', '') or '').strip():
            metadata['title'] = str(raw_meta['title']).strip()
        if str(raw_meta.get('author', '') or '').strip():
            metadata['creator'] = str(raw_meta['author']).strip()
        ...
        for page in doc:
            for block in page.get_text('blocks'):
                ...
                paragraph = _join_ocr_lines(lines)
                if paragraph:
        ...
        if not content:
            print("Error: the OCR text layer is empty — cannot build markdown")
            return None
        try:
            Path(md_path).write_text(content + '\n', encoding='utf-8')
        except Exception as e:
            print(f"Error: cannot write {md_path}: {e}")
            return None
        print(f"Extracted OCR text layer: {len(content):,} chars, "
              f"{doc.page_count} pages -> {md_path}")
        return metadata
    finally:
        doc.close()
...
def convert_html_to_markdown(html_file, md_file, strip_page_numbers=False):
    ...
    try:
        ...
        print(f"pypandoc not found. Install with: pip install pypandoc ({_import_error!r})")
...
def create_config_file(temp_dir, input_file, input_lang, output_lang, metadata=None,
                       ocr_applied=False, conversion_method='calibre_htmlz'):
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="MINERU_DIR",
        help="Ingest a MinerU output directory (mineru -p in.pdf -o MINERU_DIR) "
             "as the conversion source instead of Calibre — use for papers and "
             "formula-heavy PDFs: math stays as $...$/$$...$$ LaTeX and reading "
             "order comes from MinerU's layout model",
    )
    parser.add_argument(
        "--ocr-langs",
        default="eng+chi_sim",
        help="Tesseract languages for scanned-PDF OCR (default: eng+chi_sim)",
    )
    parser.add_argument(
        "--strip-page-numbers",
        action="store_true",
    ...
    try:
        ...
        if args.from_mineru and not os.path.exists(input_html_path):
            if not ingest_mineru_markdown(args.from_mineru, temp_dir, input_file):
                sys.exit(1)
            input_md = os.path.join(temp_dir, "input.md")
            ...
            create_config_file(temp_dir, input_file, args.ilang, args.olang,
                               _pdf_metadata(input_file), conversion_method='mineru')
            _write_source_fingerprint(temp_dir, current_fingerprint)
            print("Conversion completed successfully!")
            print(f"Temp directory: {temp_dir}")
            print(f"Markdown chunks: {chunk_count}")
            return
        ...
        if os.path.exists(input_html_path):
            ...
            create_config_file(temp_dir, input_file, args.ilang, args.olang, metadata,
                               ocr_applied=os.path.exists(os.path.join(temp_dir, _OCR_SOURCE_PDF_NAME)))
            _write_source_fingerprint(temp_dir, current_fingerprint)
            print("Conversion completed successfully!")
            print(f"Temp directory: {temp_dir}")
            return

        conversion_source = input_file
        ocr_was_applied = False
        if file_ext == '.pdf':
            text_layer = pdf_has_text_layer(input_file)
            if text_layer is False:
                print("Scanned PDF detected (no usable text layer) — OCR required")
                os.makedirs(temp_dir, exist_ok=True)
                ocr_out = os.path.join(temp_dir, _OCR_SOURCE_PDF_NAME)
                if run_ocr(input_file, ocr_out, args.ocr_langs):
                    conversion_source = ocr_out
                    ocr_was_applied = True
                else:
                    print("Continuing without OCR — conversion may yield little text.")
            elif text_layer is True:
                print("PDF text layer detected — no OCR needed")

        if ocr_was_applied:
            # Calibre's PDF input keeps the page images but drops the invisible
            # OCR text layer, so scanned books bypass the HTMLZ route and read
            # their text straight out of the OCR'd PDF.
            ocr_md = os.path.join(temp_dir, "input.md")
            metadata_ocr = extract_markdown_from_pdf(conversion_source, ocr_md)
            if metadata_ocr is None:
                sys.exit(1)
            chunk_count = _do_split_and_manifest(temp_dir, ocr_md, args.chunk_size)
            if chunk_count == 0:
                sys.exit(1)
            create_config_file(temp_dir, input_file, args.ilang, args.olang,
                               metadata_ocr, ocr_applied=True)
            _write_source_fingerprint(temp_dir, current_fingerprint)
            print("Conversion completed successfully!")
            print(f"Temp directory: {temp_dir}")
            print(f"Markdown chunks: {chunk_count}")
        ...
        if not convert_to_htmlz(conversion_source, htmlz_file, calibre_path):
            sys.exit(1)
