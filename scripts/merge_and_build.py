...
def convert_with_pandoc(md_file, html_file, title, lang_attr):
    ...
    Math parsing is disabled on the reader side: `$...$` / `$$...$$` are kept
    as literal text so MathJax (loaded by the templates) renders them. Pandoc's
    MathML output renders badly in Calibre's PDF engine, and CJK text after a
    closing `$` trips its dollar-math heuristic on some inputs.
    """
    cmd = [
...
def apply_template_to_html(html_content, template_file, output_file, title, lang_cfg, author=None):
    ...
    try:
        ...
        from publish import bundle_mathjax
        mathjax_src = bundle_mathjax(Path(output_file).parent)
        full_html = full_html.replace('$mathjax_src$', mathjax_src)
...
def html_build_fingerprint(md_file, templates, title, lang_cfg, author=None):
    """Content dependencies, including metadata/templates; timestamps are irrelevant."""
    payload = {"markdown": Path(md_file).read_text(encoding="utf-8"),
               "templates": {Path(p).name: Path(p).read_text(encoding="utf-8") for p in templates},
               "title": title, "language": lang_cfg, "author": author,
...
def convert_md_to_html(temp_dir, title, lang_cfg, author=None):
    ...
    templates = [os.path.join(SCRIPT_DIR, name) for name in ('template_ebook.html', 'template.html')]
    fingerprint = html_build_fingerprint(md_file, templates, title, lang_cfg, author)
    cache_file = Path(temp_dir) / 'html_build.json'
    book_file = os.path.join(temp_dir, 'book.html')
    if cache_file.is_file():
        try:
            cache = json.loads(cache_file.read_text(encoding='utf-8'))
            if (cache.get('fingerprint') == fingerprint and
                    all(Path(p).is_file() and _check_generated_html_sanity(p) for p in (book_doc_file, book_file))):
                print("Skipping HTML generation - all content dependencies match")
                return True
    ...
    book_doc_file = os.path.join(temp_dir, 'book_doc.html')
    if not apply_template_to_html(body_content, template_ebook, book_doc_file, title, lang_cfg, author):
        return False

    # Generate book.html with web template
    template_web = os.path.join(SCRIPT_DIR, 'template.html')
    book_file = os.path.join(temp_dir, 'book.html')
    if not apply_template_to_html(body_content, template_web, book_file, title, lang_cfg, author):
        return False
...
def find_edge():
    """Locate Microsoft Edge for headless print-to-PDF.

    Edge runs the page's JavaScript during printing, so MathJax formulas land
    in the PDF as real glyph layout — Calibre's PDF engine cannot do this
    (it neither runs JS nor renders MathML reliably).
    """
    candidates = [
        os.environ.get("EDGE_HTML_PRINT", ""),
        shutil.which("msedge"),
...
def print_html_to_pdf_via_edge(html_file, output_file, timeout=600):
    """Render an HTML file to PDF with headless Edge. Returns the output path
    or None on failure."""
    edge = find_edge()
    if not edge:
        print("Edge not found — cannot print HTML to PDF")
        return None
    uri = Path(html_file).resolve().as_uri()
    ...
    try:
        # Argument-list invocation only — no shell involved (shell=False).
        ...
        print("Edge print timed out")
        return None
    except OSError as e:
        print(f"Edge print failed to launch: {e}")
        return None
    if os.path.exists(output_file) and os.path.getsize(output_file) > 0:
        return output_file
    print(f"Edge print produced no output (exit {result.returncode})")
    return None
...
def generate_format(html_file, temp_dir, output_ext, lang_attr, cover=None):
    ...
    if output_ext == '.pdf':
        # Edge renders MathJax math correctly; Calibre does not.
        edge_out = print_html_to_pdf_via_edge(html_file, output_file)
        if edge_out:
            return output_file
        print("Falling back to Calibre for PDF (formulas will not render properly)")

    try:
        # sys.executable keeps this working on Windows, where a bare "python3"
        # may not exist or may be shadowed by a launcher stub.
...
def cleanup_intermediate_files(temp_dir):
    ...
    # Remove chunk*.md, output_chunk*.md and per-chunk meta files
    for pattern in ['chunk*.md', 'output_chunk*.md', 'output_chunk*.meta.json']:
    ...
    for name in ['input.html', 'input.md', 'output.html', 'bilingual_raw.html',
                 'ocr_source.pdf', 'bilingual.md']:
