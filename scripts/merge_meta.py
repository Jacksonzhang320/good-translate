...
def cmd_apply_merge(temp_dir):
    ...
    # Apply observations after every entity and alias has been created, so an
    # attribute of a newly proposed entity cannot be consumed and lost.
    for chunk_id, data in consumed_metas.items():
        for src in data.get('used_term_sources', []):
            term = _find_term_by_surface(glossary, src)
            if term is not None:
        ...
        for ah in data.get('attribute_hypotheses', []):
            term = _find_term_by_surface(glossary, ah.get('entity_source'))
            if term is None:
                continue
            ...
            if attribute != 'gender':
                _append_note(term, f"[attr {attribute!r}] observed {value!r} in chunk {chunk_id}; evidence={evidence!r}")
                continue
            if value not in VALID_GENDER_VALUES:
                continue
            current = term.get('gender', 'unknown')
            _append_evidence_ref(term, chunk_id)
            if term.get('gender_conflict'):
                # Conflicts persist until a deliberate glossary edit resolves
                # them. A later observation must not erase earlier disagreement.
                term['gender'] = 'unknown'
                _append_note(term, f"[gender] unresolved; observed {value!r} in chunk {chunk_id}; evidence={evidence!r}")
            elif current == 'unknown':
                term['gender'] = value
                _append_note(term, f"[gender] {value} set (chunk {chunk_id}); evidence={evidence!r}")
            elif current != value:
                term['gender'] = 'unknown'
                term['gender_conflict'] = True
                _append_note(term, f"[gender] conflict — was {current!r}, observed {value!r} in chunk {chunk_id}; evidence={evidence!r}; reverted to 'unknown'")
